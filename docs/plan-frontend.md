...
|---|--------|--------|------------|
| 0.1 | Роутинг | — | React Router: `/`, `/deeds/:id`, `/deeds/new`, `/deeds/:id/edit`, `/deeds/:id/fill`, `/records/:id`, `/widgets`, `/widgets/clicker`, `/history`, `/profile` |
...
| 0.3 | Стили и темы | 00 | Radix Themes: TextField, TextArea, Select, Checkbox, RadioGroup, Button и др. DatePicker: на десктопе — [air-datepicker](https://github.com/t1m0n/air-datepicker), на iOS/Android — нативный `input type="date"`. Время — нативный `input type="time"` (TextField.Root). CSS-переменные, mobile first, читаемость на десктопе. next-themes: тема по умолчанию — системная (prefers-color-scheme) |
| 0.4 | Обработка ошибок API | — | Единый подход: сообщения пользователю при сбоях запросов |
| 0.5 | Единый лоадер страниц | — | Компонент `PageLoading`: pageContainer + (опциональный) AppBar + центрированный текст «Загрузка…». Используется для полноэкранных состояний загрузки (список дел, просмотр дела, форма дела, история, запись, формы заполнения, глобальная загрузка приложения). Чтобы шапка не прыгала после загрузки, у `AppBar` есть невидимые резервы: `titleReserve` (пустой слот той же высоты, что `Text` size 5) и `actionsReserveCount` (невидимые слоты под `IconButton` size 3). Пробрасываются из `PageLoading`. Пример: `ClickerPage` — `PageLoading title="" backHref="/widgets" titleReserve`. |
...
| 1.8 | Сортировка | Шапка: меню порядка — по умолчанию (`created_at` с сервера), по числу «сегодня» и «всего» как на карточке; режим сохраняется в `localStorage` |
...
| 3.4 | Типы блоков в UI | Число, Текст (абзац), Один из списка, Несколько из списка, Шкала, Да/Нет, Время — [03](requirements/03-entity-blocks.md) |
...
| 4.4 | Рендер блоков по типу | Число, текст (абзац), один/несколько из списка, шкала, да/нет, время — согласно [03](requirements/03-entity-blocks.md) |
...
| 5.6 | Кнопка «Удалить запись» | Подтверждение (модалка/диалог), затем `api.records.delete(id)`, редирект к делу или к истории |
| 5.7 | Навигация | «Назад» — к странице дела или списку дел |
...
| # | Задача | Детали |
|---|--------|--------|
| 7.1 | Страница виджетов | Маршрут `/widgets`. Список доступных виджетов карточками с переходами |
| 7.2 | Виджет «Кликер» | Маршрут `/widgets/clicker`. Счётчик нажатий с сохранением записи в дело. Доступны только дела с ровно одним блоком типа «Число» |
| 7.3 | Выбор дела в кликере | Шаг 1 — выбор дела карточками; шаг 2 — Select + счётчик. Тап или удержание для инкремента/декремента (задержка 350 мс, интервал 80 мс). Счётчик не может быть отрицательным |
| 7.4 | Сохранение кликера | Кнопка «Сохранить» → `api.deeds.createRecord()` с текущими датой/временем и значением счётчика. После сохранения счётчик сбрасывается в 0 |
| 7.5 | Навигация | AppBar с кнопкой «Назад» к `/widgets` на странице кликера |

---

## Фаза 8. Полировка и правила данных

| # | Задача | Ссылка | Детали |
|---|--------|--------|--------|
| 8.1 | Поведение при редактировании дела | [13](requirements/13-rules-editing-deed-data.md) | Отображение удалённых блоков в старых записях (снимок), изменение названия/типа блока |
| 8.2 | Порядок блоков и вариантов | [05](requirements/05-rules-ordering.md) | Соблюдение `sort_order` при отображении и сохранении |
| 8.3 | Обязательные поля | [06](requirements/06-rules-required.md) | Валидация на форме заполнения и в редакторе записи |
| 8.4 | Адаптивность и доступность | 00 | Mobile first, читаемость, семантика, aria при необходимости |
...
8. **Фаза 7** — виджеты (кликер).
9. **Фаза 8** — правила данных и полировка.
...
- **next-themes**: управление светлой/тёмной темой. По умолчанию — системная (`prefers-color-scheme`). Ручной выбор сохраняется в localStorage (`log-life-theme`). Интеграция с Radix Themes через `attribute="class"`.
- **Agentation** (dev): инструмент для аннотирования UI-элементов. Позволяет кликать по элементам, добавлять заметки и передавать структурированный контекст (CSS-селекторы, пути к файлам, иерархию React) в AI-агенты (Cursor, Claude Code и др.). Подключается в `main.tsx`, отображается только в dev (`import.meta.env.DEV`). Опционально: MCP-сервер (`npx agentation-mcp server`) для real-time синхронизации с Cursor.
